...
bool is_invariant(spot::twa_graph_ptr aut, bdd &condition)
{
  ...
  if (aut->state_is_accepting(init))
    return false;
  if (!aut->state_is_accepting(other))
    return false;
...
int composition_mt::epilogue(std::string synth_fname)
{
  ...
  if ((!r.solved) || not_fully_solved)
  {
    ...
    if (not_fully_solved)
      verb_do(1, vout << "Solved but not with the right invariant -> extra solve\n");
    solve_game(r);
  }

  verb_do(0, vout << "AUTOMATA\n"; spot::print_hoa(utils::vout, r.aut, nullptr); vout << "\nAUTOMATAEND\n");
  if (r.safe != nullptr)
    verb_do(0, vout << "ANTICHAIN\n"
                    << *r.safe << "ANTICHAINEND\n");
  if (r.unsafe != nullptr)
    verb_do(0, vout << "UNREALANTICHAIN\n"
                    << *r.unsafe << "UNREALANTICHAINEND\n");
...
int composition_mt::run(int worker_count, std::string synth_fname)
{
  ...
  while (active_workers > 0)
  {
    ...
    {
      if (base_remaining == 0)
      { // once all formula jobs are finished, we know all invariants
        base_remaining = -1;
        finish_invariant();
      }
    }
...
safety_game composition_mt::prepare_formula(spot::formula f, bool check_real, unreal_x_t opt_unreal_x)
{
  ...
  // verb_do (0, vout<<"AUTOMATA\n";spot::print_hoa (utils::vout, aut, nullptr); vout<<"\nAUTOMATAEND\n");
