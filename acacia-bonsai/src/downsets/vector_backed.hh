...
namespace downsets
{
  ...
  template <typename Vector>
  inline std::ostream &operator<<(std::ostream &os, const vector_backed<Vector> &f)
  {
    for (auto &&el : f)
      os << el << std::endl;
...
namespace upsets
{
  ...
  public:
    ...
    bool insert(Vector &&v)
    {
      ...
      {
        auto res = v.partial_order(*it);
        if (not must_remove and res.geq())
        { // v is dominated.
          // if must_remove is true, since we started with an antichain,
          // it's not possible that res.geq () holds.  Hence we don't check for
          // geq if must_remove is true.
          return false;
        }
        else if (res.leq())
        {                     // v dominates *it
